use crate::contract::{
    AcceptedContract, AdaptorInfo, ClosedContract, Contract, ContractInfo, ContractView,
    ContractViewInfo, FailedAcceptContract, FailedSignContract, FundingInputInfo, OfferedContract,
    OracleView, SignedContract,
};
use crate::serialization::get_tx_input_infos;
use bitcoin::{
...
impl From<crate::serialization::Error> for Error {
    fn from(e: crate::serialization::Error) -> Error {
...
pub trait Network {
    fn send_message(&self, message: DlcMessage) -> Result<(), Error>;
}

pub struct DlcDaemon<W: Deref, B: Deref, S: DerefMut, O: Deref, T: Deref>
where
    W::Target: Wallet,
    B::Target: Blockchain,
    S::Target: DlcStorage,
...
impl<W: Deref, B: Deref, S: DerefMut, O: Deref, T: Deref> DlcDaemon<W, B, S, O, T>
where
    W::Target: Wallet,
    B::Target: Blockchain,
    S::Target: DlcStorage,
    ...
    T::Target: Time,
{
    ...
    fn get_party_params(
        ...
        let utxos =
            self.wallet
                .get_utxos_for_amount(appr_required_amount, Some(fee_rate), Some(true))?;

        let mut funding_inputs_info: Vec<FundingInputInfo> = Vec::new();
        let mut funding_tx_info: Vec<TxInputInfo> = Vec::new();
        let mut total_input = 0;
        for utxo in utxos {
            let prev_tx = self.blockchain.get_transaction(&utxo.outpoint.txid)?;
    ...
    fn get_oracle_announcements(
        &self,
        oracle_views: &OracleView,
    ) -> Result<Vec<OracleAnnouncement>, Error> {
        let mut announcements = Vec::new();
        for pubkey in &oracle_views.public_keys {
            let oracle = self.oracles.get(pubkey).ok_or(Error::InvalidParameters)?;
            announcements.push(oracle.get_announcement(&oracle_views.event_id)?.clone());
        }
    ...
    fn contract_view_info_to_contract_info(
        &self,
        contract_view_info: &ContractViewInfo,
    ...
    pub fn send_offer(&mut self, contract: &ContractView) -> Result<OfferDlc, Error> {
    ...
    pub fn on_offer_message(&mut self, offered_message: &OfferDlc) -> Result<(), Error> {
    ...
    pub fn on_accept_message(&mut self, accept_msg: &AcceptDlc) -> Result<DlcMessage, Error> {
        ...
        println!("CHECKING ACCEPT REFUND: {:?}", refund_verify_result);
    ...
    pub fn on_sign_message(&mut self, sign_message: &SignDlc) -> Result<(), Error> {
