...
impl ContractInfoInner {
    pub fn get_payouts(&self, total_collateral: u64) -> Vec<Payout> {
        self.contract_descriptor.get_payouts(total_collateral)
    }
}
...
impl ContractDescriptor {
    pub fn get_payouts(&self, total_collateral: u64) -> Vec<Payout> {
        match self {
            ContractDescriptor::ContractDescriptorV0(v0) => v0
                .payouts
                .iter()
                .map(|x| x.get_payout(total_collateral))
                .collect(),
            ContractDescriptor::ContractDescriptorV1(v1) => unimplemented!(),
        }
    }
}
