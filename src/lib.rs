...
pub fn get_secp_committed_key(oracle_pub_key: PublicKey,
                              ...
                              message: &[u8; 32]) -> PublicKey {
    let s = Secp256k1::new();

    let message_hash = sha256d::Hash::hash(message);
...
pub fn get_committed_key(oracle_pub_key: PublicKey,
    ...
    let pubkey_list: Vec<secp256k1::key::PublicKey> =
        oracle_r_points
            .iter()
            .zip(messages.iter())
            .map( |(r, msg)| get_secp_committed_key(oracle_pub_key, *r, msg) ).collect();
...
// TODO - instead of copy and paste from RL, the function should be called by including RL
// before this can be done, RL should use the same library version of secp256k1
...
mod tests {
    ...
    #[test]
    fn create_cet_redeem_script_test() {
        let secp = Secp256k1::new();
        let secret_key = SecretKey::from_slice(&[0xcd; 32]).expect(
            "32 bytes, within curve order"
        );
        ...
        let msg = &[2u8; 32];
        let script = create_cet_redeem_script(
            PublicKey::from_secret_key(&secp, &secret_key),
            PublicKey::from_secret_key(&secp, &secret_key),
            PublicKey::from_secret_key(&secp, &secret_key),
            PublicKey::from_secret_key(&secp, &secret_key),
            &vec![PublicKey::from_secret_key(&secp, &secret_key)],
            vec![msg],
            1
        );
        ...
        assert_eq!(
            &format!("{:x}", script),
            ""
        );
