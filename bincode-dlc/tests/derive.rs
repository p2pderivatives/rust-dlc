...
fn test_encodable() {
    ...
    assert_eq!(&slice[..bytes_written], &[0, 0, 0, 5, 0, 0, 0, 10, 20]);
}
...
fn test_decodable() {
    ...
    let slice = [0, 0, 0, 5, 0, 0, 0, 10, 0, 0, 4, 0];
...
fn test_abcd() {
    ...
    let ser = [2, 104, 104, 0, 0, 0, 0, 0, 0, 0, 10];
    let mut slice = [0u8; 11];
    bincode::encode_into_slice(t, &mut slice).unwrap();
    assert_eq!(ser, slice);
}
