...
use dlc::{channel::sub_channel::SplitTx, DlcChannelId, SubChannelId};
use lightning::{
    chain::{
...
use crate::{channel::party_points::PartyBasePoints, error::Error};
...
pub struct SubChannel {
    /// The ID for the subchannel.
    pub channel_id: SubChannelId,
...
impl AcceptedSubChannel {
    fn get_dlc_channel_id(
        ...
        DlcChannelId::from_bytes(crate::utils::compute_id(
            self.split_tx.transaction.txid(),
            channel_idx as u16 + 1,
            &temporary_channel_id.inner(),
        ))
    }
}
...
/// Generate a temporary channel id for a DLC channel based on the subchannel id, the update index of the
...
pub fn generate_temporary_dlc_channel_id(
    ...
    let bytes = bitcoin::hashes::sha256::Hash::hash(&data).into_inner();

    DlcChannelId::from_bytes(bytes)
}
