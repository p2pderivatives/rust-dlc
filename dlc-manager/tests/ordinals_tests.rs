...
use dlc_manager::{ContractId, Oracle, Storage};
...
fn ord_binary_path() -> PathBuf {
    ...
    if !dir.to_str().unwrap().ends_with("dlc-manager") {
        dir.push("dlc-manager");
    }
    dir.push("tests");
    dir.push("ord");
    dir
...
fn get_ord_test_params(
    ...
    let oracles = match outcome_descriptor {
        OrdOutcomeDescriptor::Enum(_) => {
            let threshold = if add_attestation { threshold } else { 0 };
            get_enum_oracles(nb_oracles, threshold)
        }
    ...
    let contract_descriptor = get_ord_contract_descriptor(
        ordinal_sat_point,
        ordinal_tx,
        outcome_descriptor,
        refund_offer,
...
fn ordinal_enum_test() {
    ...
    let (mut alice_manager, mut bob_manager, sink_rpc, inscription, test_params) =
        init(&outcome_descriptor, true, true);
...
#[test]
#[ignore]
fn ordinal_enum_accept_owner_test() {
    let outcome_descriptor = get_enum_ord_outcome_descriptor(false);
    let (mut alice_manager, mut bob_manager, sink_rpc, inscription, test_params) =
        init(&outcome_descriptor, false, true);
...
#[test]
#[ignore]
fn ordinal_enum_refund_offer_test() {
    let outcome_descriptor = get_enum_ord_outcome_descriptor(false);
    let (mut alice_manager, mut bob_manager, sink_rpc, inscription, test_params) =
        init(&outcome_descriptor, true, false);

    let contract_id = execute_contract(
        &mut alice_manager,
        &mut bob_manager,
        &sink_rpc,
        &test_params,
    ...
    let contract = alice_manager
        .get_store()
        .get_contract(&contract_id)
        .unwrap();

    if let Some(Contract::Refunded(_)) = contract {
    } else {
        panic!("Expected to be in refunded state but was: {:?}", contract);
    }

    let offer_inscriptions = get_inscriptions(OFFER_PARTY);

    let inscription = offer_inscriptions
        .iter()
        .find(|x| x.inscription == inscription.inscription)
        .expect("To find the inscription");
    let outpoint = location_string_to_outpoint(&inscription.location);
    let tx = sink_rpc.get_raw_transaction(&outpoint.txid, None).unwrap();
    assert_eq!(DEFAULT_POSTAGE + OFFER_COLLATERAL, tx.output[0].value);
    assert_eq!(ACCEPT_COLLATERAL, tx.output[1].value);
}

#[test]
#[ignore]
fn ordinal_enum_refund_accept_test() {
    let outcome_descriptor = get_enum_ord_outcome_descriptor(false);
    let (mut alice_manager, mut bob_manager, sink_rpc, inscription, test_params) =
        init(&outcome_descriptor, false, false);
    ...
    let accept_inscriptions = get_inscriptions(ACCEPT_PARTY);

    let inscription = accept_inscriptions
        .iter()
        .find(|x| x.inscription == inscription.inscription)
        .expect("To find the inscription");
    let outpoint = location_string_to_outpoint(&inscription.location);
    let tx = sink_rpc.get_raw_transaction(&outpoint.txid, None).unwrap();
    assert_eq!(DEFAULT_POSTAGE + ACCEPT_COLLATERAL, tx.output[0].value);
    assert_eq!(OFFER_COLLATERAL, tx.output[1].value);
}
...
fn init(
    ...
    let (owner, owner_rpc) = if is_offer_owner {
        (OFFER_PARTY, &alice_rpc)
    } else {
        (ACCEPT_PARTY, &bob_rpc)
    };
    inscribe_logo(owner);
    generate_blocks(&sink_rpc, 1);
    let mut inscriptions = get_inscriptions(owner);
    ...
    owner_rpc.lock_unspent(&inscriptions_outpoints).unwrap();
    ...
    let test_params = get_ord_test_params(
        1,
        1,
        sat_point,
        &ordinal_tx,
...
fn execute_contract(
    ...
    let (contract_id, _, accept_msg) = bob_manager
